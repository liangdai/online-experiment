crtrate_50 = rep(NA,itertimes)
exp_reg_50 = rep(NA,itertimes)
post_reg_50 = rep(NA,itertimes)
sum_reg_50 = rep(NA,itertimes)
...
for(i in 1:num_time){
	delta = alpha1_store[iter,i]+beta1_store[iter,i]-alpha2_store[iter,i]-beta2_store[iter,i]
	prob_1 = min(1,max(0,(num_batch-delta)/2/num_batch))
	#if (stop==0)
...
crtrate_50[ldai]=(sum(correct_rpm))
sum_reg_50[ldai]=(mean(rowSums(loss_rpm)+post_loss))
exp_reg_50[ldai]=(mean(rowSums(loss_rpm)))
post_reg_50[ldai]=(mean(post_loss))
}
