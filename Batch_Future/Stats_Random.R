...
rdm_exp_power = pow(alpha1_store[,],beta1_store[,],alpha2_store[,],beta2_store[,],0.05)
rdm_exp_ci = ci(alpha1_store[,],beta1_store[,],alpha2_store[,],beta2_store[,])

rdm_winprob = matrix(0,num_iter,num_time+1) 
for(i in 1:num_iter)
for(j in 1:(num_time+1)){
rdm_winprob[i,j] = max(compute.win.prob(c(alpha1_store[i,j]-1,alpha2_store[i,j]-1),c(alpha1_store[i,j]+beta1_store[i,j]-1,alpha2_store[i,j]+beta2_store[i,j]-1),1000))
}

RDM_Power = colMeans(rdm_exp_power)
RDM_CI = colMeans(rdm_exp_ci)
RDM_Winprob = colMeans(rdm_winprob)
RDM_Power_mean = mean(rdm_exp_power[,(num_time+1)])
RDM_Power_sd = sd(rdm_exp_power[,(num_time+1)])
RDM_CI_mean = mean(rdm_exp_ci[,(num_time+1)])
RDM_CI_sd = sd(rdm_exp_ci[,(num_time+1)])
RDM_Winprob_mean = mean(rdm_winprob[,(num_time+1)])
RDM_Winprob_sd = sd(rdm_winprob[,(num_time+1)])
