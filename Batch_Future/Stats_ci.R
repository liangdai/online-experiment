...
for(i in 1:num_time){
	...
	prob_1 = sqrt(pp1*(1-pp1))/(sqrt(pp1*(1-pp1))+sqrt(pp2*(1-pp2)))
	left1 = prob_1*(num_time*num_batch)-(alpha1_store[iter,i]+beta1_store[iter,i])
	left2 = (1-prob_1)*(num_time*num_batch)-(alpha2_store[iter,i]+beta2_store[iter,i])
	prob_1 = left1/(left1+left2)
...
ci_exp_power = pow(alpha1_store[,],beta1_store[,],alpha2_store[,],beta2_store[,],0.05)
ci_exp_ci = ci(alpha1_store[,],beta1_store[,],alpha2_store[,],beta2_store[,])

ci_winprob = matrix(0,num_iter,num_time+1) 
for(i in 1:num_iter)
for(j in 1:(num_time+1)){
ci_winprob[i,j] = max(compute.win.prob(c(alpha1_store[i,j]-1,alpha2_store[i,j]-1),c(alpha1_store[i,j]+beta1_store[i,j]-1,alpha2_store[i,j]+beta2_store[i,j]-1),1000))
}

CI_Power = colMeans(ci_exp_power)
CI_CI = colMeans(ci_exp_ci)
CI_Winprob = colMeans(ci_winprob)
CI_Power_mean = mean(ci_exp_power[,(num_time+1)])
CI_Power_sd = sd(ci_exp_power[,(num_time+1)])
CI_CI_mean = mean(ci_exp_ci[,(num_time+1)])
CI_CI_sd = sd(ci_exp_ci[,(num_time+1)])
CI_Winprob_mean = mean(ci_winprob[,(num_time+1)])
CI_Winprob_sd = sd(ci_winprob[,(num_time+1)])

mean((alpha1_store[,num_time+1]+beta1_store[,num_time+1])/(alpha2_store[,num_time+1]+beta2_store[,num_time+1]))

#alpha1_store[,num_time+1]/(alpha1_store[,num_time+1]+beta1_store[,num_time+1])
#alpha2_store[,num_time+1]/(alpha2_store[,num_time+1]+beta2_store[,num_time+1])
#(alpha1_store[,num_time+1]+beta1_store[,num_time+1])/(alpha2_store[,num_time+1]+beta2_store[,num_time+1])