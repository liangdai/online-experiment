...
info_exp_power = pow(alpha1_store[,],beta1_store[,],alpha2_store[,],beta2_store[,],0.05)
info_exp_ci = ci(alpha1_store[,],beta1_store[,],alpha2_store[,],beta2_store[,])

info_winprob = matrix(0,num_iter,num_time+1) 
for(i in 1:num_iter)
for(j in 1:(num_time+1)){
info_winprob[i,j] = max(compute.win.prob(c(alpha1_store[i,j]-1,alpha2_store[i,j]-1),c(alpha1_store[i,j]+beta1_store[i,j]-1,alpha2_store[i,j]+beta2_store[i,j]-1),1000))
}

LIN_Power = colMeans(info_exp_power)
LIN_CI = colMeans(info_exp_ci)
LIN_Winprob = colMeans(info_winprob)
LIN_Power_mean = mean(info_exp_power[,(num_time+1)])
LIN_Power_sd = sd(info_exp_power[,(num_time+1)])
LIN_CI_mean = mean(info_exp_ci[,(num_time+1)])
LIN_CI_sd = sd(info_exp_ci[,(num_time+1)])
LIN_Winprob_mean = mean(info_winprob[,(num_time+1)])
LIN_Winprob_sd = sd(info_winprob[,(num_time+1)])
