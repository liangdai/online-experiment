pow2 = function(alpha1,beta1,alpha2,beta2,n1,n2,z){
...
for(i in 1:num_time){
	...
	powers = pow2(alpha1_store[iter,i],beta1_store[iter,i],alpha2_store[iter,i],beta2_store[iter,i],1:(num_time*num_batch-1),(num_time*num_batch-1):1,0.05)
	prob_1 = which(powers==max(powers))/(num_time*num_batch)
	#ppp=c(ppp,(alpha1_store[iter,i]/beta1_store[iter,i])/(alpha2_store[iter,i]/beta2_store[iter,i]))
...
pwr_exp_power = pow(alpha1_store[,],beta1_store[,],alpha2_store[,],beta2_store[,],0.05)
pwr_exp_ci = ci(alpha1_store[,],beta1_store[,],alpha2_store[,],beta2_store[,])

pwr_winprob = matrix(0,num_iter,num_time+1) 
for(i in 1:num_iter)
for(j in 1:(num_time+1)){
pwr_winprob[i,j] = max(compute.win.prob(c(alpha1_store[i,j]-1,alpha2_store[i,j]-1),c(alpha1_store[i,j]+beta1_store[i,j]-1,alpha2_store[i,j]+beta2_store[i,j]-1),1000))
}

PWR_Power = colMeans(pwr_exp_power)
PWR_CI = colMeans(pwr_exp_ci)
PWR_Winprob = colMeans(pwr_winprob)
PWR_Power_mean = mean(pwr_exp_power[,(num_time+1)])
PWR_Power_sd = sd(pwr_exp_power[,(num_time+1)])
PWR_CI_mean = mean(pwr_exp_ci[,(num_time+1)])
PWR_CI_sd = sd(pwr_exp_ci[,(num_time+1)])
PWR_Winprob_mean = mean(pwr_winprob[,(num_time+1)])
PWR_Winprob_sd = sd(pwr_winprob[,(num_time+1)])
