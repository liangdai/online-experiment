...
grd_exp_power = pow(alpha1_store[,],beta1_store[,],alpha2_store[,],beta2_store[,],0.05)
grd_exp_ci = ci(alpha1_store[,],beta1_store[,],alpha2_store[,],beta2_store[,])

grd_winprob = matrix(0,num_iter,num_time+1) 
for(i in 1:num_iter)
for(j in 1:(num_time+1)){
grd_winprob[i,j] = max(compute.win.prob(c(alpha1_store[i,j]-1,alpha2_store[i,j]-1),c(alpha1_store[i,j]+beta1_store[i,j]-1,alpha2_store[i,j]+beta2_store[i,j]-1),1000))
}

GRD_Power = colMeans(grd_exp_power)
GRD_CI = colMeans(grd_exp_ci)
GRD_Winprob = colMeans(grd_winprob)
GRD_Power_mean = mean(grd_exp_power[,(num_time+1)])
GRD_Power_sd = sd(grd_exp_power[,(num_time+1)])
GRD_CI_mean = mean(grd_exp_ci[,(num_time+1)])
GRD_CI_sd = sd(grd_exp_ci[,(num_time+1)])
GRD_Winprob_mean = mean(grd_winprob[,(num_time+1)])
GRD_Winprob_sd = sd(grd_winprob[,(num_time+1)])
